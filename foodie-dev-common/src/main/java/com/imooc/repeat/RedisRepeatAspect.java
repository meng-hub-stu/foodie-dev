...
import com.imooc.repeat.util.AspectUtil;
import com.imooc.repeat.util.IpUtil;
...
@Component
public class RedisRepeatAspect {
    ...
    public Object redisRepeat(ProceedingJoinPoint point, RedisRepeat repeat) throws Throwable {

        MethodSignature signature = (MethodSignature) point.getSignature();
        Method method = signature.getMethod();

        // 通过 AnnotationUtils.findAnnotation 获取 RateLimiter 注解
        RedisRepeat rateLimiter = AnnotationUtils.findAnnotation(method, RedisRepeat.class);
        if (rateLimiter != null) {
            // limit:类名:方法名
