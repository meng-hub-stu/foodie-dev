...
import com.imooc.exception.RedisRuntimeException;
...
public class RedisLockAspect {
    ...
    public Object lockAroundAction(ProceedingJoinPoint point, RedisLockApi redisLockApi) throws Throwable {
        //获取参数
        ...
        //获取注解中的参数
        int retryNum = redisLockApi.retryNum();
        long retryWait = redisLockApi.retryWait();
        long lockWait = redisLockApi.lockWait();
        long autoUnlockTime = redisLockApi.autoUnlockTime();
        String lockPrefix = redisLockApi.lockPrefix();
        if (StringUtils.isBlank(lockPrefix)) {
            lockPrefix = className + DOT + methodSignature.getName();
        }
        //获取参数的el数据
        String el = parseByEl(redisLockApi.lockParameter(), methodSignature.getParameterNames(), parameterValues);
        //设置锁的名称
        String lockName = lockPrefix;
        if (StringUtils.isNotBlank(redisLockApi.lockParameter())){
            lockName = lockPrefix + COLON + el;
        }
        RLock lock = this.getLockByModel(redisLockApi.lockModel(), lockPrefix, el);
        try {
